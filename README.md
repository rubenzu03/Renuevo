...
Renuevo is server-action driven for the web UI, and exposes one public HTTP endpoint:
the automation entry point used by schedulers.

### Architecture

Web actions delegate to a shared domain layer, so business rules never drift:
