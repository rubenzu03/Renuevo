import { beforeEach, describe, expect, it, vi } from "vitest";

const { isMobileAuthorizedMock } = vi.hoisted(() => ({
  isMobileAuthorizedMock: vi.fn(),
}));

vi.mock("@/lib/mobile-auth", () => ({
  isMobileAuthorized: isMobileAuthorizedMock,
}));

const { prismaMock } = vi.hoisted(() => ({
  prismaMock: {
    subscription: {
      findUnique: vi.fn(),
      update: vi.fn(),
      delete: vi.fn(),
    },
    priceHistory: { create: vi.fn() },
    $transaction: vi.fn(),
  },
}));

vi.mock("@/lib/prisma", () => ({ prisma: prismaMock }));
vi.mock("@/lib/mobile/http", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/mobile/http")>();
  return { ...actual };
});

import { NextRequest } from "next/server";
import { DELETE, GET, PUT } from "./route";

function decimal(value: string) {
  return { toString: () => value, equals: (v: string) => value === v };
}

function subscription(overrides: Record<string, unknown> = {}) {
  return {
    id: "sub_1",
    name: "Netflix",
    priceCurrent: decimal("9.99"),
    currency: "EUR",
    billingCycle: "monthly",
    billingIntervalDays: null,
    nextRenewalDate: new Date("2026-09-01T00:00:00Z"),
    category: "streaming",
    isActive: true,
    createdAt: new Date("2026-08-01T00:00:00Z"),
    updatedAt: new Date("2026-08-01T00:00:00Z"),
    ...overrides,
  };
}

function request(url: string, method: string, body?: unknown): NextRequest {
  const req = new NextRequest(url, {
    method,
    body: body === undefined ? undefined : JSON.stringify(body),
  });
  req.headers.set("authorization", "Bearer token");
  return req;
}

const ctx = { params: Promise.resolve({ id: "sub_1" }) };

describe("GET /api/mobile/subscriptions/:id", () => {
  beforeEach(() => {
    isMobileAuthorizedMock.mockReset();
    isMobileAuthorizedMock.mockReturnValue(true);
    prismaMock.subscription.findUnique.mockReset();
  });

  it("rejects unauthenticated requests", async () => {
    isMobileAuthorizedMock.mockReturnValue(false);
    const res = await GET(request("http://localhost:3000/x", "GET"), ctx);
    expect(res.status).toBe(401);
  });

  it("returns 404 for missing subscriptions", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(null);
    const res = await GET(request("http://localhost:3000/x", "GET"), ctx);
    expect(res.status).toBe(404);
  });

  it("returns the subscription with price history", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(
      subscription({
        priceHistories: [
          {
            id: "ph_1",
            price: decimal("12.99"),
            recordedAt: new Date("2026-07-01T00:00:00Z"),
          },
        ],
      })
    );
    const res = await GET(request("http://localhost:3000/x", "GET"), ctx);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.priceHistory).toEqual([
      { id: "ph_1", price: "12.99", recordedAt: "2026-07-01T00:00:00.000Z" },
    ]);
  });
});

describe("PUT /api/mobile/subscriptions/:id", () => {
  beforeEach(() => {
    isMobileAuthorizedMock.mockReset();
    isMobileAuthorizedMock.mockReturnValue(true);
    prismaMock.subscription.findUnique.mockReset();
    prismaMock.subscription.update.mockReset();
    prismaMock.priceHistory.create.mockReset();
    prismaMock.$transaction.mockReset();
  });

  it("rejects unauthenticated requests", async () => {
    isMobileAuthorizedMock.mockReturnValue(false);
    const res = await PUT(request("http://localhost:3000/x", "PUT", {}), ctx);
    expect(res.status).toBe(401);
  });

  it("returns 404 for missing subscriptions", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(null);
    const res = await PUT(
      request("http://localhost:3000/x", "PUT", {
        name: "Netflix",
        price: "10.99",
        currency: "EUR",
        billingCycle: "monthly",
        nextRenewalDate: "2026-09-01",
      }),
      ctx
    );
    expect(res.status).toBe(404);
  });

  it("records price history when the price changes", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(subscription());
    prismaMock.$transaction.mockImplementation(
      async (fn: (tx: typeof prismaMock) => Promise<unknown>) =>
        fn(prismaMock)
    );
    prismaMock.subscription.update.mockResolvedValue(
      subscription({ priceCurrent: decimal("10.99") })
    );
    const res = await PUT(
      request("http://localhost:3000/x", "PUT", {
        name: "Netflix",
        price: "10.99",
        currency: "EUR",
        billingCycle: "monthly",
        nextRenewalDate: "2026-09-01",
      }),
      ctx
    );
    expect(res.status).toBe(200);
    expect(prismaMock.priceHistory.create).toHaveBeenCalledWith({
      data: { subscriptionId: "sub_1", price: expect.anything() },
    });
  });

  it("does not record price history when the price is unchanged", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(subscription());
    prismaMock.$transaction.mockImplementation(
      async (fn: (tx: typeof prismaMock) => Promise<unknown>) =>
        fn(prismaMock)
    );
    prismaMock.subscription.update.mockResolvedValue(subscription());
    const res = await PUT(
      request("http://localhost:3000/x", "PUT", {
        name: "Netflix",
        price: "9.99",
        currency: "EUR",
        billingCycle: "monthly",
        nextRenewalDate: "2026-09-01",
      }),
      ctx
    );
    expect(res.status).toBe(200);
    expect(prismaMock.priceHistory.create).not.toHaveBeenCalled();
  });

  it("rejects invalid payloads", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(subscription());
    const res = await PUT(
      request("http://localhost:3000/x", "PUT", { name: "" }),
      ctx
    );
    expect(res.status).toBe(400);
  });
});

describe("DELETE /api/mobile/subscriptions/:id", () => {
  beforeEach(() => {
    isMobileAuthorizedMock.mockReset();
    isMobileAuthorizedMock.mockReturnValue(true);
    prismaMock.subscription.findUnique.mockReset();
    prismaMock.subscription.delete.mockReset();
  });

  it("rejects unauthenticated requests", async () => {
    isMobileAuthorizedMock.mockReturnValue(false);
    const res = await DELETE(request("http://localhost:3000/x", "DELETE"), ctx);
    expect(res.status).toBe(401);
  });

  it("returns 404 for missing subscriptions", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(null);
    const res = await DELETE(request("http://localhost:3000/x", "DELETE"), ctx);
    expect(res.status).toBe(404);
  });

  it("deletes and returns 204", async () => {
    prismaMock.subscription.findUnique.mockResolvedValue(subscription());
    const res = await DELETE(request("http://localhost:3000/x", "DELETE"), ctx);
    expect(res.status).toBe(204);
    expect(prismaMock.subscription.delete).toHaveBeenCalledWith({
      where: { id: "sub_1" },
    });
  });
});